...
def home(posts, shops):
    ...
    latest = posts[:10]
    entry = [('/about/','About','Line cook, chipped Shun, Tojiro DP, Japan, then Adrichops.'),('/maker-map/','Maker map','Interactive regional graph of smiths, sharpeners, workshops, brands and line relationships.'),('/blog/','Blog','All articles, reviews, guides, maker essays and recommended-shop notes in one place.'),('/knife-finder/','Knife Finder','Recommendations, starter paths and maintenance pairings.'),('/disclosure/','Disclosure','Affiliate policy, source discipline and correction rules.')]
    ...
    return head('Adrichops — Japanese knives, sharpening and practical buying notes', 'My personal knife notebook: reviews, maker context, sharpening, boards, stones and recommendations.', '/') + header('') + f'''<main class="page">
  <section class="hero">
  ...
  <section class="section"><div class="section-head"><div><span class="kicker">Start here</span><h2>First notes to read.</h2></div><a class="text-link" href="/blog/">All articles</a></div>{cards(start_posts)}</section>
  <section class="section"><div class="section-head"><div><span class="kicker">Recommended shops</span><h2>Retailers with useful source trails.</h2></div><a class="text-link" href="/blog/">In the blog</a></div>{shop_cards(shops[:3])}</section>
  <section class="section"><div class="section-head"><div><span class="kicker">Finder</span><h2>One recommendation, plus the upkeep.</h2></div><a class="text-link" href="/knife-finder/">Open Knife Finder</a></div>{finder_markup()}</section>
  <section class="section"><div class="section-head"><div><span class="kicker">Notebook database</span><h2>Latest notes.</h2></div><div class="section-actions"><button class="button" type="button" data-search-open>Search</button><a class="button" href="/blog/">Blog</a></div></div>{article_summary_list(latest)}</section>
</main>''' + footer()
...
def blog_page(posts):
    return collection_page('Blog', 'All Adrichops articles, reviews, maker notes, sharpening guides and recommended-shop notes in one archive.', 'Blog', posts, '/blog/', 'Everything written on the site lives here now: buying guides, review briefs, maker essays, sharpening notes, maintenance notes and source-led recommendations.')
...
def recommendations_page(posts):
    ...
    return head('Knife Finder — Adrichops', 'Knife Finder, starter kits, maintenance pairings and affiliate-ready buying notes.', '/knife-finder/') + header('Knife Finder') + f'''<main class="page"><section class="collection-hero"><span class="kicker">Knife Finder</span><h1>Choose the knife and the upkeep together.</h1><p>The Finder returns a knife direction, steel direction, blade length, caveats and maintenance kit. Low-fuss vegetable prep can point to a VG10 nakiri, simple stainless nakiri or light Asian cleaver / cai dao. Bread, tomato and serrated jobs can point to a Victorinox-style bread knife with storage and board caveats.</p></section>{finder_markup()}<section class="section"><div class="section-head"><div><span class="kicker">Starter paths</span><h2>Useful next reads.</h2></div></div>{cards(rec_posts)}</section></main>''' + footer()
...
def main():
    ...
    write('/knife-finder/', recommendations_page(posts))
    ...
    write('/recommendations/', redirect_page('/knife-finder/', 'Knife Finder'))
    write('/recommendations.html', redirect_page('/knife-finder/', 'Knife Finder'))
    write('/knife-finder.html', redirect_page('/knife-finder/', 'Knife Finder'))
